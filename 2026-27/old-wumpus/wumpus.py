...
def setup():
    ...
    for p in product(range(NUM_ROWS), range(NUM_COLS)):
        ...
        solver.add(STENCH[p] == z3.Or(WUMPUS[n] for n in neighbors))
        solver.add(BREEZE[p] == z3.Or(PIT[n] for n in neighbors))
        solver.add(GOLD[p] == z3.Or(GLARE[n] for n in neighbors))
...
def check(function, fact: dict[tuple, z3.BoolRef], knowledge: z3.Solver) -> set[tuple[int, int]]:
    ok = set()
    for p in product(range(NUM_ROWS), range(NUM_COLS)):
...
def create_model(knowledge: z3.Solver) -> list[list[str]]:
    ...
    for _ in range(NUM_ROWS):
        model.append([MAP_EMPTY] * NUM_COLS)

    # Check for the Wumpus
    wumpus = check(k.is_entailed, WUMPUS, knowledge)
    possible_wumpus = check(k.is_consistent, WUMPUS, knowledge)
    ic(wumpus, possible_wumpus)
    assert len(wumpus) <= 1
    assert wumpus <= possible_wumpus

    # Check for the pit
    pit = check(k.is_entailed, PIT, knowledge)
    possible_pit = check(k.is_consistent, PIT, knowledge)
    ic(pit, possible_pit)
    assert len(pit) <= 1
    assert pit <= possible_pit

    safe = check(k.is_entailed, SAFE, knowledge)
    ...
    for p in possible_wumpus & possible_pit:
        model[p[0]][p[1]] = MAP_MAYBE_WUMPUS_OR_PIT
    for p in possible_wumpus - possible_pit:
        model[p[0]][p[1]] = MAP_MAYBE_WUMPUS
    for p in possible_pit - possible_wumpus:
        model[p[0]][p[1]] = MAP_MAYBE_PIT
