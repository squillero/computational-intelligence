...
class State():
    ...
    def move_group(self, source_name: str, destination_name: str, group: StudentGroup) -> State:
        ...
        if source_name == 'bike': # getting off the bike
            result_bike -= group
            if destination_name == 'pizzeria': result_pizzera += group
            else: result_pub += group
        else: # mounting on the bike
            result_bike += group
            if source_name == 'pizzeria': result_pizzera -= group
            else: result_pub -= group
        ...
        else: result_bike_position = self.bike_position
